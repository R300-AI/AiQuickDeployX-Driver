...
class YOLOv8_Uploader():
    def __init__(self, client, user, dtype, task):
        self.client, self.user, self.dtype, self.task = client, user, dtype, task
    ...
    def Upload(self, dataset_path, remain_folder=False):
    ...
    def upload_systeminfo(self, dataset_path):
        ...
        info = {"user": self.user, "dtype": self.dtype, "task": self.task, "nc": data['nc'], "names": data['names'], "features": data['features']}
