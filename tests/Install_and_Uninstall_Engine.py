...
# 寫入MongoDB後原檔會自動刪除(可用remain_origin=True保留資料集路徑的原檔)
###############################################
#dataset_path = 'data\HardHat'
#client = MongoDB('localhost', '27017', 'admin', 'admin')
#client.Push(dtype='Vision2D', task='ObjectDetection', dataset_path=dataset_path, remain_origin = False)
...
###############################################
client = MongoDB('localhost', '27017', 'admin', 'admin')#print available datasets

driver = Plugins(dtype='Vision2D', task='ObjectDetection')#print available  models
driver.Load('Pytorch/YOLOv8n')

#對應規格匹配
client.Pull(dtype='Vision2D', task='ObjectDetection', dataset='HardHat', engine=engine, username='markov')
